    ...
    <Tabs vertical>
        ...
        <Content
        v-if="users"
        :title="$t('adminPanel.titleUsers')"
        :inputTitle="$t('adminPanel.inputUserTitle')"
        type="users"
        :dataInvoices="users"
        ...
        <td class="table__data paddingSmall">
            <nuxt-link class="table__link" :to="`/adminPanel/${data._id}`">
            <p>{{ data.username }}</p></nuxt-link
            >
        </td>
        <td class="table__data paddingSmall">
            <p>
        ...
        <td
            class="table__data paddingSmall"
        ...
        >
            ...
            :hideArrow="true"
            :selectedOptionProp="
            bannedOrActive(data.is_active, data.banned)
            "
            />
        </td>
        </template>
        ...
        <!-- tab Deal -->
        <Content
        :key="componentInvoice"
        v-if="invoiceBoolean"
        :title="$t('adminPanel.deal')"
        :inputTitle="$t('adminPanel.inputDealTitle')"
        type="deal"
        ...
        <td class="table__data paddingSmall">
            ...
            <p style="font-size: 10px; padding: 0; line-height: 15px">
            ID сделки: {{ data._id }}
            </p>
        </td>
        <td class="table__data paddingSmall">
            <p>{{ data.buyer_username }} - {{ data.seller_username }}</p>
        </td>
        <td class="table__data paddingSmall">
            <p>{{ data.amount_usdt }} - USDT</p>
        </td>
        <td class="table__data paddingSmall">
            {{ data.status }}
        </td>
        <td class="table__data paddingNull" style="text-align: center">
            <Button
            :disabled="disabledBtnFreeze(data.status)"
            :class="disabledBtnFreeze(data.status) ? null : 'green'"
            @click.native="freeze(data)"
            style="
            border-radius: 50%;
            padding: 0;
            height: 40px;
        ...
        <td class="table__data paddingNull" style="text-align: center">
            <Button
            :disabled="disabledBtnCancel(data.status)"
            :class="disabledBtnCancel(data.status) ? null : 'green'"
            @click.native="cancel(data)"
            style="
            border-radius: 50%;
            padding: 0;
            height: 40px;
        ...
        <td class="table__data paddingNull" style="text-align: center">
            <Button
            :disabled="disabledBtnConfirm(data.status)"
            :class="disabledBtnConfirm(data.status) ? null : 'green'"
            @click.native="confirm(data)"
            style="
            border-radius: 50%;
            padding: 0;
            height: 40px;
    ...
    async freeze(data) {
        ...
        if (res.status == 200) {
        this.$store.dispatch('adminPanel/editStatus', {
        id: data._id,
        status: 'frozen',
        })
        }
    ...
    async confirm(data) {
        ...
        .put(`/admin/invoice/${data._id}/transfer/`)
        .then((e) => {
        this.$store.dispatch('adminPanel/editStatus', {
        id: data._id,
        status: 'completed',
    ...
    async created() {
    this.$store.dispatch('adminPanel/getInvoices').then(() => {
